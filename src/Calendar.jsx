...
for (let hour = 7; hour <= 22; hour++) {
  ...
  lines.push(
    <div
      className="bg-emerald-900 h-px col-span-full col-start-2"
...
export default function Calendar({ events }) {
  ...
  for (const [colNum, day] of dayMap) {
    for (const block of events[day]) {
      ...
      cal.push(
        <div
          className={`col-start-${colNum} bg-amber-200 text-slate-900 text-center text-sm flex rounded-lg drop-shadow-md drop-shadow-gray-800 transition hover:shadow-sm hover:shadow-gray-900 hover:scale-105 border border-black `}
  ...
  return (
    <div className="grid grid-cols-[3rem_repeat(7,_minmax(0,_1fr))] grid-rows-[minmax(0,_1fr)_repeat(192,_5px)] auto-cols-max gap-x-1 w-full sm:w-2/3">
      ...
      <div
        className={`col-start-${
          dayOfWeek + 2
        } row-span-full bg-emerald-900 rounded opacity-25 -z-50`}
      ></div>
      {nowLine}
    </div>
  );
